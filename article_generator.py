...
Article:"""
        ...
        if "deepseek" in self.model_name.lower():
            ...
            # DeepSeek works better with very direct, simple prompts
        ...
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    ...
                    # Use very simple prompt on retries
...
Article:"""
                    ...
                    # Use full prompt on first attempt (if not DeepSeek)
                ...
                if response and len(response) > 50:
                    ...
                    if self._is_article_complete(response):
                        ...
                        print("⚠️ Article appears incomplete, will retry with different approach")
                        if attempt == max_retries:
                            # Last attempt - return what we have if it's substantial
                            if len(response) > 500:
                ...
                if attempt == max_retries:
                    # Last attempt failed, suggest specific solutions based on model
                    if "deepseek" in self.model_name.lower():
    ...
    def _is_article_complete(self, text: str) -> bool:
        ...
        # Check if last line looks like a conclusion
        ...
        # If it's substantial content (>1000 chars) and doesn't seem cut off, consider it complete
    ...
    def _call_ollama(self, prompt: str, temperature: float = 0.7, max_tokens: int = 5000) -> str:
        ...
        # Set timeout based on model and content length
        ...
        # Adjust timeout based on expected output length
        if max_tokens > 1500:
            timeout += 60  # Add 1 minute for longer outputs
        ...
        options = {
            ...
            "stop": [],  # Remove stop tokens that might cut content short
            "top_p": 0.9,
            "repeat_penalty": 1.1
        }
        
        # Special settings for DeepSeek models
        if "deepseek" in self.model_name.lower():
            options.update({
                "temperature": min(temperature, 0.3),  # DeepSeek works better with lower temp
                "top_p": 0.8,
                "repeat_penalty": 1.05,
                "top_k": 20,
                "stop": [],  # No stop tokens for DeepSeek - let it complete naturally
            })
            print(f"🧠 Using DeepSeek-optimized parameters: temp={options['temperature']}")
        else:
            # For other models, only use very specific stop tokens to prevent cutoffs
            options["stop"] = ["---END---", "###END###"]  # Only use explicit end markers
        ...
        try:
            ...
            # Handle DeepSeek-R1's thinking format - extract content after </think>
            ...
            if "<think>" in generated_text and "</think>" in generated_text:
                ...
                if think_end != -1:
                    ...
                    if article_content:
                        ...
                        # If nothing after </think>, try to extract from before
                        print("⚠️ No content found after </think>, using full response")
            elif "<think>" in generated_text:
                # Incomplete thinking block - model might have been cut off
                print("⚠️ Incomplete thinking block detected, extracting what we can")
                # Remove the incomplete thinking part
                ...
                if think_start > 0:
                    ...
                    # Thinking is at the start but no end - this is problematic
            ...
            # Clean up any remaining XML-style tags that might leak through
            import re
            ...
            if (last_sentence and not last_sentence.endswith(('.', '!', '?', '"', "'")) and 
                ...
                len(last_sentence.split()[-1]) < 20):  # Last "word" is reasonably short
                print(f"⚠️ Content appears cut off (ends with: '{last_sentence[-50:]}')")
                # Try to find a good stopping point
                sentences = generated_text.split('.')
                if len(sentences) > 1:
                    # Remove the incomplete last sentence
                    complete_content = '.'.join(sentences[:-1]) + '.'
                    print(f"✂️ Trimmed incomplete sentence: {len(generated_text)} -> {len(complete_content)} chars")
